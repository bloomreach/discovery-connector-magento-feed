...
async function openGitlabPR(repoUrl, releaseVersion) {
  ...
  if (!projectId) {
    console.warn(
      "Could not log in or find project id. Rerun the command or debug the issue."
    );
    process.exit(1);
  }
...
async function openGitPR(repoUrl, releaseVersion, showLogIn) {
  ...
  if (!loggedIn) {
    ...
    const fn =
      'document?.body?.getAttribute("class").split(" ").find(c => c.startsWith("logged-in"))';
...
async function run(repoUrl, repoType) {
  ...
  if (
    shell.exec("git status --porcelain=v1 2>/dev/null | wc -l | grep 0", {
      silent: true,
    }).code !== 0
  ) {
    console.error(
